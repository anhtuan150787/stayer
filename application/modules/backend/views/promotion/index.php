                        ...
                        <?php if (!empty($records)) : ?>
                            <?php foreach ($records as $v) : ?>
                                <tr>
                                    ...
                                    <td><?php echo '<strong>' . $v->date_apply_from . '</strong>' . ' đến ' . '<strong>' . $v->date_apply_to . '</strong>';?></td>
                                    <td>
                                        <?php
                                        $room_id = explode(',', $v->room_id);
                                        $room_string = '';
                                        foreach($rooms as $room)
                                        {
                                            if (in_array($room->id, $room_id))
                                                $room_string .= ',' . html_escape($room->name); 
                                        }
                                        echo trim($room_string, ',');
                                        ?>
                                    </td>
                                    <td><?php echo $v->create_time; ?> bởi <strong><?php echo $v->admin_username; ?></strong></td>
                                    <td>
                                        <?php echo $v->update_time; ?> bởi <strong><?php echo $v->update_by; ?></strong>
                                    </td>
                                    <td>
                                        <?php
                                        if ($v->status == 1)
