...
class National extends Backend_Controller
{

    protected $limit = 20;
    
    protected $model = 'national_model';
    ...
    public function __construct()
    {
        ...
        $this->data['breadcrumbs'] = array(
            ...
            'Quốc gia' => site_url() . '/backend/' . $this->data['controller'],
        );
        
        /*
         * Load js
         */
        $this->data['js'] = array('national.js');

        /*
         * Xoa hinh tam
         */

        $last_image_time = $this->image_tmp_model->find_all_by(array('type' => 'national', 'create_time <' => time()));
    ...
    public function index()
    {
         ...
         * Data view
         */
        $records = $this->{$this->model}->where(array('deleted' => 0))->limit($limit, $offset)->find_all();
    ...
    {   
        ...
        else
        {
            ...
            if ($params['picture'] != '' && $params['picture'] != $record->picture)
            {
                ...
                unlink($this->config->item('pic_path') . 'nationals/' . $record->picture);
            }
    ...
    public function save_image($img_name)
    {
        ...
        $des = $this->config->item('pic_path') . 'nationals/' . $img_name;
