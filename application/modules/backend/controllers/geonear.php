...
class Geonear extends Backend_Controller
{

    protected $limit = 20;
    
    protected $model = 'geonear_model';
    ...
    public function __construct()
    {
        ...
        $this->data['breadcrumbs'] = array(
            ...
            'Khu vực' => site_url() . '/backend/' . $this->data['controller'],
        );
        
        /*
         * Load js
         */
        $this->data['js'] = array('geonear.js');
    }
    ...
    public function index()
    {
         ...
         * Data view
         */
        $this->{$this->model}->select('geonear.*, province.name as province_name, district.name as district_name');
        $this->{$this->model}->join('province', 'geonear.province_id = province.id', 'left');
        $this->{$this->model}->join('district', 'geonear.district_id = district.id', 'left');
        $this->{$this->model}->where(array('geonear.deleted' => 0));
        $this->{$this->model}->order_by('geonear.id', 'desc');
